  ...
  return (
    <div className="relative font-sans" ref={dropdownRef}>
      ...
      <div 
        ...
        className="min-h-[52px] bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 flex flex-wrap gap-2 items-center cursor-text transition-all focus-within:ring-2 focus-within:ring-[#0061ff]/30 focus-within:border-[#0061ff]/50"
      >
        {selectedLocations.length === 0 && (
          <span className="text-slate-400 text-[11px] font-black uppercase tracking-widest pl-2">Select Regions, Districts or Divisions...</span>
        )}
        
        {selectedLocations.map(loc => (
          <div key={loc} className="bg-[#0061ff] text-white px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm shadow-blue-500/20">
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute top-[calc(100%+8px)] left-0 w-full bg-white border border-slate-100 rounded-2xl shadow-2xl shadow-slate-900/10 max-h-[300px] overflow-y-auto z-50 p-2 animate-in fade-in slide-in-from-top-2">
          {filteredOptions.length === 0 ? (
             <div className="p-4 text-center text-slate-400 text-xs font-bold">No areas found.</div>
          ) : (
            <div className="space-y-1">
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isSelected ? "bg-blue-50 text-[#0061ff]" : "hover:bg-slate-50 text-slate-700"
                    )}
                  >
                    <div className="flex flex-col">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        {opt.parent && <span className="text-[9px] font-bold text-slate-400 italic">in {opt.parent}</span>}
                      </div>
                    </div>
                    
                    <div className={cn(
                      "w-5 h-5 rounded-md border flex items-center justify-center transition-all",
                      isSelected ? "bg-[#0061ff] border-[#0061ff] text-white" : "border-slate-200 bg-white"
                    )}>
              ...
              {filteredOptions.length > 100 && (
                <div className="text-center p-2 text-xs font-bold text-slate-400">
