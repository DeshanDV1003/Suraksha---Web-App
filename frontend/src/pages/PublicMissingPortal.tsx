...
export default function PublicMissingPortal() {
  ...
  const inputClass = "w-full bg-slate-50 dark:bg-[#0a1628] border border-slate-200 dark:border-white/10 text-slate-800 dark:text-white/90 placeholder-slate-400 dark:placeholder-slate-600 p-4 rounded-xl font-medium focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/40 outline-none transition-all"
    ...
    return (
      <div className="min-h-screen bg-slate-100 dark:bg-[#0e1d36] flex items-center justify-center p-4">
        <div className="bg-white dark:bg-[#131f33] border border-slate-200 dark:border-white/10 p-10 rounded-[2rem] shadow-2xl max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-black text-slate-800 dark:text-white/90 mb-2">{t('public_missing_portal.report_submitted')}</h2>
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-[#0e1d36] p-4 sm:p-8 font-sans">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="text-3xl font-black text-slate-800 dark:text-white/90">{t('public_missing_portal.portal_title')}</h1>
        ...
        {/* Card */}
        <div className="bg-white dark:bg-[#131f33] border border-slate-200 dark:border-white/10 rounded-[2rem] shadow-2xl overflow-hidden">
          ...
          <div className="flex border-b border-slate-200 dark:border-white/10">
            <button
              ...
              className={cn(
                ...
                activeTab === 'search'
                  ? 'bg-blue-500/15 text-blue-600 dark:text-blue-300 border-b-2 border-blue-500'
          ...
          <div className="p-6 sm:p-10">
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {filteredPersons.length === 0 ? (
                      <div className="col-span-2 py-16 text-center bg-slate-50 dark:bg-white/5 rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
                        <UserSearch className="w-10 h-10 text-slate-400 dark:text-slate-600 mx-auto mb-3" />
                      ...
                      filteredPersons.map(person => (
                        <div key={person.id} className="bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-5 hover:bg-slate-100 dark:hover:bg-white/8 hover:border-slate-300 dark:hover:border-white/20 transition-all">
                          <div className="flex justify-between items-start mb-3 gap-2">
                            <h3 className="font-black text-base text-slate-800 dark:text-white/90 leading-tight">
                          ...
                          <div className="space-y-1.5 text-sm">
                            ...
                            <p className="flex gap-2 items-start text-slate-600 dark:text-slate-300">
                              <MapPin className="w-4 h-4 text-blue-400 shrink-0 mt-0.5" />
                              <span><span className="font-bold text-slate-500">{t('public_missing_portal.last_seen_label')}</span> {person.lastSeen}</span>
                            </p>
                            <p className="text-slate-500 dark:text-slate-400 line-clamp-2 mt-1">{person.description}</p>
                          </div>
            ...
            {activeTab === 'report' && (
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="border-t border-slate-200 dark:border-white/10 pt-6 space-y-5">
                  <h4 className="font-bold text-slate-700 dark:text-white/80">{t('public_missing_portal.contact_section')}</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
