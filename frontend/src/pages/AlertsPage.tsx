    ...
    <div className="space-y-8 animate-in fade-in duration-500 font-sans pb-20">
      ...
      {canBroadcast && (
        <div className="p-10 border-4 border-[#0061ff]/10 rounded-[3.5rem] bg-gradient-to-br from-blue-50/30 to-white space-y-8 backdrop-blur-xl">
          <div className="flex items-center gap-4 text-[#0061ff]">
             <div className="p-3 bg-[#0061ff] rounded-2xl text-white shadow-lg shadow-blue-500/20"><Bell className="w-6 h-6" /></div>
             <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
            {categories.map((cat, i) => (
              ...
              >
                <div className={cn("p-4 rounded-2xl mb-4 group-hover:scale-110 transition-transform bg-slate-50", cat.color)}><cat.icon className="w-7 h-7" /></div>
            ...
            <form onSubmit={handleBroadcast} className="space-y-10">
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1 italic">Transmission Channels</label>
                  <div className="flex flex-wrap gap-3">
                    {[
                      ...
                      <button 
                        ...
                        className={cn("flex items-center gap-2 px-4 py-2.5 rounded-xl border text-[11px] font-black uppercase tracking-widest transition-all", newAlert.channels[ch.id as keyof typeof newAlert.channels] ? "bg-blue-50 border-blue-500 text-blue-700 shadow-sm" : "bg-white border-slate-200 text-slate-400 hover:bg-slate-50")}
                      >
                ...
                <div className="grid grid-cols-2 gap-8">
                   ...
                   <div className="space-y-3">
                     ...
                     <select className="suraksha-input appearance-none bg-slate-50 font-black text-[11px] uppercase tracking-widest" value={newAlert.type} onChange={(e) => setNewAlert({...newAlert, type: e.target.value})}>
                  ...
                  {newAlert.translated && (
                    <div className="grid grid-cols-2 gap-4 mt-2 animate-in slide-in-from-top-2">
                      <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                         <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Sinhala Translation</div>
                      ...
                      <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                         <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Tamil Translation</div>
                ...
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] pl-1 italic">Schedule (Optional)</label>
              ...
              <button disabled={isSubmitting} className="suraksha-button w-full h-20 text-lg flex items-center justify-center gap-4 transition-all uppercase tracking-widest font-black">
                 ...
                 <><div className="p-2 bg-white/20 rounded-xl"><Send className="w-6 h-6" /></div>{newAlert.scheduledTime ? 'Schedule Directive' : t('alerts.new_alert')}</>}
              </button>
            </form>
          </div>
        )}
             ...
             <div className="p-8 bg-slate-900 rounded-[3rem] shadow-2xl relative overflow-hidden flex-1 flex flex-col justify-center min-h-[500px]">
                ...
                <div className="relative z-10 space-y-6">
                   <div className="text-center space-y-2 mb-10">
                      <span className="text-[10px] font-black text-[#0061ff] uppercase tracking-[0.3em]">Omni-Channel Sync Preview</span>
                   ...
                   <div className="max-w-sm mx-auto w-full bg-white p-8 rounded-[2.5rem] shadow-[0_40px_60px_-15px_rgba(0,0,0,0.5)] border border-slate-100 animate-in zoom-in-95 duration-500">
                      <div className="flex items-center gap-4 mb-6">
                         <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-xl transition-colors duration-500", newAlert.type === 'EMERGENCY' ? 'bg-red-500' : newAlert.type === 'WARNING' ? 'bg-amber-500' : 'bg-[#0061ff]')}>
                           <Bell className="w-6 h-6" />
                         </div>
                         <div>
                           <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Suraksha Node</div>
                      ...
                      <div className="flex gap-2 flex-wrap mb-6">
                        {Object.entries(newAlert.channels).map(([k, v]) => v && (
                           <span key={k} className="bg-slate-100 text-slate-500 text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded">✓ {k}</span>
                        ))}
                      </div>
      ...
      <div className="space-y-10 pt-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
           <div>
             ...
             <div className="flex gap-4 mt-4">
               <button onClick={() => setActiveTab('ACTIVE')} className={cn("text-[11px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition-colors", activeTab === 'ACTIVE' ? 'bg-blue-100 text-blue-700' : 'text-slate-400 hover:bg-slate-50')}>Active & History</button>
               <button onClick={() => setActiveTab('SCHEDULED')} className={cn("text-[11px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition-colors", activeTab === 'SCHEDULED' ? 'bg-amber-100 text-amber-700' : 'text-slate-400 hover:bg-slate-50')}>Scheduled (Mock)</button>
             </div>
           </div>
           ...
           <div className="flex items-center gap-4">
              <div className="relative group w-full md:w-64">
                 ...
                 <input type="text" placeholder="Search logs..." className="suraksha-input pl-12 h-12 bg-white" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
              <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-2 focus:ring-[#0061ff]/20 cursor-pointer h-12">
        ...
        <div className="grid grid-cols-1 gap-6">
          {loading ? (
             <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100 shadow-sm"><p className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] animate-pulse">Decrypting Sync Logs...</p></div>
          ) : filteredAlerts.length === 0 ? (
            <div className="text-center py-24 bg-white rounded-[3rem] border-2 border-dashed border-slate-200">
               <div className="p-6 bg-slate-50 inline-block rounded-full mb-4"><XCircle className="w-10 h-10 text-slate-300" /></div>
            ...
            filteredAlerts.map((alert, i) => (
              <div key={alert.id} className="suraksha-card p-8 flex flex-col md:flex-row md:items-start bg-white hover:shadow-2xl hover:shadow-blue-500/5 transition-all group overflow-hidden border-none shadow-sm relative">
                ...
                <div className="flex-1 space-y-4">
                  ...
                  <p className="text-sm font-bold text-slate-500 max-w-3xl">{alert.message}</p>
                  ...
                  {alert.active && (
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mt-4">
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2"><BarChart2 className="w-3 h-3" /> Community Feedback (Real-time)</div>
                      <div className="flex gap-4 h-12">
                         {getMockFeedbackData(alert.id).map((data, idx) => (
                           <div key={idx} className="flex-1 flex flex-col justify-end bg-white border border-slate-100 rounded-xl p-2 relative overflow-hidden group/chart">
                              ...
                              <div className="relative z-10 flex justify-between items-baseline">
                                <span className="text-[9px] font-black text-slate-500 uppercase">{data.name}</span>
      ...
      {isMapModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white rounded-[2rem] p-8 max-w-xl w-full shadow-2xl relative">
            ...
            <p className="text-xs font-bold text-slate-500 mb-6">Mock Interface: Simulating geospatial polygon drawing for targeted alerts.</p>
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setIsMapModalOpen(false)} className="px-6 py-3 bg-slate-100 text-slate-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-200">Cancel</button>
              <button onClick={() => {
