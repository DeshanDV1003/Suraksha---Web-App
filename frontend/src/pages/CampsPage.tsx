  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 font-sans pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {loading ? (
          <div className="lg:col-span-2 flex flex-col items-center justify-center py-20 bg-white rounded-[1.5rem] shadow-sm space-y-3">
            ...
            return (
              <div key={idx} className="suraksha-card p-7 flex flex-col space-y-6 hover:shadow-lg transition-all relative group rounded-[1.5rem]">
                ...
                <div className="pt-6 border-t border-slate-50 flex items-center justify-between mt-auto">
                  ...
                  <button onClick={() => setSelectedCampId(camp.id)} className="text-sm font-bold text-[#0061ff] hover:underline">
                    Manage Camp
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[1.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleAddCamp} className="p-8 space-y-6">
               ...
               <div className="pt-4 flex gap-4">
                 <button type="button" onClick={() => setShowModal(false)} className="flex-1 px-6 py-4 bg-slate-100 text-slate-600 rounded-2xl text-sm font-bold hover:bg-slate-200 transition-all">Cancel</button>
                 <button type="submit" disabled={isSubmitting} className="flex-1 px-6 py-4 bg-[#0061ff] text-white rounded-2xl text-sm font-bold shadow-lg shadow-blue-500/25">Create Camp</button>
               </div>
            </form>
          </div>
        </div>
...
function CampDetailsModal({ campId, onClose }: { campId: string, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-6xl rounded-[1.5rem] shadow-2xl flex flex-col h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="px-8 py-6 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold mt-1 flex items-center gap-1"><MapPin className="w-3 h-3"/> {camp.location}</p>
          </div>
          <button onClick={onClose} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><X className="w-5 h-5"/></button>
        </div>

        {/* Tabs Navigation */}
        <div className="flex px-8 border-b border-slate-100 shrink-0 overflow-x-auto">
          {[
            ...
            <button
              ...
              className={cn("flex items-center gap-2 px-6 py-4 text-[11px] font-black uppercase tracking-widest border-b-2 whitespace-nowrap", activeTab === t.id ? "border-[#0061ff] text-[#0061ff]" : "border-transparent text-slate-400 hover:text-slate-600")}
            >
          ...
          {activeTab === 'OVERVIEW' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="suraksha-card p-6 bg-white space-y-6">
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Current Occupancy</h4>
                    <div className="text-3xl font-black text-[#1e293b]">{camp.currentOccupancy} <span className="text-lg text-slate-400">/ {camp.totalCapacity}</span></div>
                  ...
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Transfers</h4>
                    <div className="flex gap-4">
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex-1 text-center">
                        ...
                        <div className="text-[9px] uppercase tracking-widest text-slate-400">Pending Inbound</div>
                      </div>
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 flex-1 text-center">
                        <div className="text-2xl font-black text-orange-600">{camp.transfersOut?.length || 0}</div>
                        <div className="text-[9px] uppercase tracking-widest text-slate-400">Pending Outbound</div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="suraksha-card overflow-hidden h-64 lg:h-auto bg-slate-100 relative min-h-[300px]">
...
function ResidentsTab({ campId, residents, refresh }: any) {
  ...
  return (
    <div className="space-y-6">
      <form onSubmit={handleAdd} className="suraksha-card p-6 bg-white flex flex-col md:flex-row items-end gap-4 shadow-sm border-blue-100">
         ...
         <div className="flex-1 w-full space-y-1">
           <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">NIC (Optional)</label>
      ...
      <div className="suraksha-card bg-white overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Name</th>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">NIC</th>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Check-In</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {residents.map((r: any) => (
              <tr key={r.id}>
                ...
                <td className="p-4 text-slate-500">{r.nic || '-'}</td>
                <td className="p-4 text-slate-500">{new Date(r.checkInTime).toLocaleString()}</td>
            ...
            {residents.length === 0 && (
              <tr><td colSpan={4} className="p-8 text-center text-slate-400 italic font-bold">No active residents in digital roll.</td></tr>
            )}
          </tbody>
        </table>
      </div>
         ...
         return (
           <div key={type} className={cn("suraksha-card p-6 border-2 transition-colors", color)}>
              ...
              <div className="flex gap-2">
                <button onClick={() => handleUpdate(type, (inv.quantity + 50).toString())} className="flex-1 bg-white/50 hover:bg-white text-xs font-black uppercase tracking-widest py-2 rounded-xl transition-colors shadow-sm">+50</button>
                <button onClick={() => handleUpdate(type, Math.max(0, inv.quantity - 10).toString())} className="flex-1 bg-white/50 hover:bg-white text-xs font-black uppercase tracking-widest py-2 rounded-xl transition-colors shadow-sm">-10</button>
              </div>
           </div>
         )
       })}
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
       <div className="lg:col-span-1">
         <form onSubmit={handleAdd} className="suraksha-card p-6 bg-white space-y-4 shadow-sm">
       ...
       <div className="lg:col-span-2">
         <div className="suraksha-card bg-white shadow-sm p-6 space-y-4">
           {schedules.map((s: any) => (
             <div key={s.id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{s.type} • Till {s.endTime}</span>
                 </div>
               </div>
               <button onClick={() => handleDelete(s.id)} className="text-slate-400 hover:text-red-500"><X className="w-5 h-5"/></button>
             </div>
           ))}
           {schedules.length === 0 && <p className="text-center text-slate-400 italic text-sm font-bold py-10">No schedules posted for today.</p>}
         </div>
       </div>
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="suraksha-card bg-white overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-slate-50">
            <tr>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Patient</th>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Hospital</th>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Severity</th>
              <th className="p-4 text-left text-[10px] font-black uppercase tracking-widest text-slate-400">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
            {referrals.map((r: any) => (
              <tr key={r.id}>
                ...
                <td className="p-4 text-slate-500">{r.hospitalAssigned}</td>
                <td className="p-4">
                  <span className={cn("text-[10px] font-black px-2 py-1 rounded uppercase tracking-widest", r.conditionSeverity === 'CRITICAL' ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-600')}>{r.conditionSeverity}</span>
                </td>
                <td className="p-4">
                   <select 
            ...
            {referrals.length === 0 && (
              <tr><td colSpan={4} className="p-8 text-center text-slate-400 italic font-bold">No hospital referrals logged.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function TransferModal({ fromCamp, onClose, refresh }: any) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-md rounded-[1.5rem] p-8 shadow-2xl">
        <div className="flex justify-between items-center mb-6">
           <h3 className="text-xl font-black text-[#1e293b] flex items-center gap-2"><ArrowRightLeft className="w-5 h-5"/> Transfer Request</h3>
           <button onClick={onClose}><X className="w-5 h-5 text-slate-400"/></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
           <div className="space-y-1">
             <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Destination Camp</label>
           ...
           <div className="space-y-1">
             <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Number of People</label>
           ...
           <div className="pt-4 flex gap-4">
             <button type="button" onClick={onClose} className="flex-1 bg-slate-100 text-slate-600 rounded-xl py-3 font-bold text-sm">Cancel</button>
