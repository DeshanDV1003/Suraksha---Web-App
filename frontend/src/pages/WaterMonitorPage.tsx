...
const alertColor = (lvl: string) => {
  switch (lvl) {
    case 'CRITICAL':    case 'MAJOR_FLOOD': return { bg: 'bg-red-500/15',    border: 'border-red-500/30',    text: 'text-red-400',    badge: 'bg-red-500/20 text-red-300 border-red-500/30' };
    case 'WARNING':     case 'MINOR_FLOOD': return { bg: 'bg-amber-500/15',  border: 'border-amber-500/30',  text: 'text-amber-400',  badge: 'bg-amber-500/20 text-amber-300 border-amber-500/30' };
    case 'WATCH':       case 'ALERT':       return { bg: 'bg-yellow-400/10', border: 'border-yellow-400/30', text: 'text-yellow-400', badge: 'bg-yellow-400/15 text-yellow-300 border-yellow-400/30' };
    default:                                return { bg: 'bg-cyan-400/5',    border: 'border-cyan-400/10',   text: 'text-cyan-400',   badge: 'bg-cyan-400/10 text-cyan-300 border-cyan-400/20' };
  }
};
const confColor = (c: number) => c >= 0.85 ? 'text-emerald-400' : c >= 0.75 ? 'text-cyan-400' : 'text-amber-400';
const trendIcon = (t: string, chg: number) => {
  if (t === 'RISING' || chg > 0.05) return <span className="text-red-400">↑</span>;
  if (t === 'FALLING' || chg < -0.05) return <span className="text-emerald-400">↓</span>;
...
function LevelBar({ current, watch, minor, major, predicted }: {
  ...
  return (
    <div className="relative w-full h-3 bg-white/5 rounded-full overflow-visible mt-1">
      ...
      {predicted !== undefined && (
        <div className="absolute top-[-3px] w-[3px] h-[18px] rounded bg-white/60"
...
export default function WaterMonitorPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── Tabs ── */}
      <div className="flex gap-2 border-b border-cyan-400/10">
        ...
        <div>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-52 rounded-2xl bg-[#131f33] border border-cyan-400/10 animate-pulse" />
              ))}
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {predictions.map(p => {
                ...
                return (
                  <div key={p.gaugeId}
                    className={`p-5 rounded-2xl border ${c.bg} ${c.border} bg-[#131f33] transition-all hover:border-opacity-50 relative overflow-hidden`}>
                    {/* Glow blob */}
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="text-sm font-bold text-slate-100">{p.riverName}</div>
                        <div className="text-xs text-slate-400">{p.stationName} · {p.district}</div>
                      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {riverData.slice(0, 20).map((r, i) => {
                  ...
                  return (
                    <tr key={i} className={`border-b border-cyan-400/5 hover:bg-white/[0.02] ${r.status === 'MAJOR_FLOOD' ? 'bg-red-500/5' : ''}`}>
                      <td className="px-4 py-3">
                        <div className="font-semibold text-slate-100">{r.riverName}</div>
                        <div className="text-xs text-slate-400">{r.stationName}</div>
                      </td>
                      <td className="px-4 py-3 text-slate-400">{r.district}</td>
                      <td className="px-4 py-3">
                      ...
                      <td className={`px-4 py-3 text-sm font-semibold ${r.changeFromLastHour > 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                        {r.changeFromLastHour > 0 ? '+' : ''}{r.changeFromLastHour.toFixed(2)}m
                      </td>
                      <td className="px-4 py-3">{trendIcon(r.trend, r.changeFromLastHour)}</td>
                      <td className="px-4 py-3 text-slate-400 text-xs">{r.alertLevel.toFixed(1)}m</td>
                      <td className="px-4 py-3">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rainfallData.slice(0, 20).map((s, i) => {
                  ...
                  return (
                    <tr key={i} className="border-b border-cyan-400/5 hover:bg-white/[0.02]">
                      <td className="px-4 py-3 font-semibold text-slate-100">{s.stationName}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-200 font-semibold w-14 text-right">{s.rainfallMmPerHour.toFixed(1)}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-300">{s.cumulativeRain24h.toFixed(1)}</td>
                      <td className="px-4 py-3">
      ...
      {!mlStatus.online && (
        <div className="p-5 rounded-2xl border border-amber-500/20 bg-amber-500/5">
          ...
          <div className="text-xs text-slate-400 space-y-1 font-mono">
            ...
            <div>3. <span className="text-cyan-300">uvicorn main:app --port 8000</span>  ← starts prediction API</div>
            <div className="pt-1 text-slate-500">Then click "Run AI Prediction" above to see LSTM predictions.</div>
          </div>
        </div>
      )}
    </div>
